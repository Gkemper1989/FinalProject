# This code was developed by myself using some hints from third sources like stackoverflow, w3school,
# python documentation and google frameworks documentation.
#
# Sumarizing the Problem and constraints
...
def main():
    ...
    for c in all_carer:
        for d in all_days:
            for s in all_shifts:
                shifts[(c, d, s)] = model.NewBoolVar('shift_n%id%is%i' % (c, d, s))
                # the array define assignments for
    ...
    for d in all_days:
        for s in all_shifts:
            model.AddExactlyOne(shifts[(c, d, s)] for c in all_carer)
            # the sum of carers assigned to the shift s is 1.

    # Each carer works at most one shift per day.
    for c in all_carer:
        for d in all_days:
            model.AddAtMostOne(shifts[(c, d, s)] for s in all_shifts)
            # the sum of shifts assigned to that carer is at most 1.
    ...
    # floor division to garantee the maximun number of complete shifts per carer
    min_shifts_per_carer = (num_shifts * num_days) // num_carers
